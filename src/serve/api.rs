        ...
        if b == b'%' {
            ...
            if let (Some(h1), Some(h2)) = (h1, h2) {
                ...
                if let Ok(hex_s) = std::str::from_utf8(&hex_str) {
                    if let Ok(byte) = u8::from_str_radix(hex_s, 16) {
                        bytes.push(byte);
                        continue;
                    }
                }
...
impl<S, T> axum::extract::FromRequestParts<S> for SafePath<T>
where
    T: serde::de::DeserializeOwned + Send,
    S: Send + Sync,
{
    type Rejection = (StatusCode, axum::response::Html<String>);
    ...
    async fn from_request_parts(
        ...
        if let Ok(raw_params) = axum::extract::RawPathParams::from_request_parts(parts, state).await
        {
            for (_, val) in raw_params.iter() {
                let decoded_str = percent_decode_lossy(val);
                let cleaned = decoded_str.trim_end_matches('/').to_string();
            ...
            let raw_segs: Vec<&str> = path
                ...
                .collect();
            let param_segs =
                if raw_segs.first() == Some(&"notes") || raw_segs.first() == Some(&"raw") {
                    &raw_segs[1..]
                } else {
                    &raw_segs[..]
                };
            for seg in param_segs {
                let decoded_str = percent_decode_lossy(seg);
                let cleaned = decoded_str.trim_end_matches('/').to_string();
                segments.push(cleaned);
            }
        }

        let val = if segments.len() == 1 {
            serde_json::Value::String(segments.into_iter().next().unwrap())
        } else {
        ...
        serde_json::from_value(val).map(SafePath).map_err(|_| {
            (
...
/// Decode legacy percent-encoded Unicode codepoints
/// generated by the previous non-standard url_encode implementation.
fn decode_legacy_codepoints(s: &str) -> String {
    ...
    while let Some(c) = chars.next() {
        if c == '%' {
            ...
            if hex_buf.len() == 4 {
                if let Ok(code) = u32::from_str_radix(&hex_buf, 16) {
...
/// Resolve a requested file slug to an actual public file name in the given note.
/// Supports exact match, .md extension match, legacy codepoint demangling, and prefix/suffix matching.
pub fn resolve_public_filename(
    state: &ServerState,
    note_id: &str,
    file_slug: &str,
) -> Option<String> {
    ...
    let candidate = if file_slug.ends_with(".md") {
        file_slug.to_string()
    } else {
        format!("{}.md", file_slug)
    };
    if state.chat_db.is_file_public(note_id, &candidate) {
    ...
    let clean_slug = file_slug.strip_suffix(".md").unwrap_or(file_slug);

    // 2. Case-insensitive exact slug match
    for fname in &public_files {
        let fslug = fname.strip_suffix(".md").unwrap_or(fname);
        if fslug.eq_ignore_ascii_case(clean_slug) {
            return Some(fname.clone());
        }
    }

    // 3. Legacy Codepoint Demangling
    let demangled = decode_legacy_codepoints(clean_slug);
    let demangled_candidate = format!("{}.md", demangled);
    if state.chat_db.is_file_public(note_id, &demangled_candidate) {
        return Some(demangled_candidate);
    }
    for fname in &public_files {
        let fslug = fname.strip_suffix(".md").unwrap_or(fname);
        if fslug.eq_ignore_ascii_case(&demangled) {
            return Some(fname.clone());
        }
    }

    // 4. Prefix / Suffix match fallback for corrupted bytes
    let mut best_match: Option<String> = None;
    for fname in &public_files {
        let fslug = fname.strip_suffix(".md").unwrap_or(fname);

        let prefix_end = clean_slug
            .char_indices()
            .find(|(_, c)| !c.is_ascii_alphanumeric() && *c != '-')
            .map(|(idx, _)| idx)
            .unwrap_or_else(|| clean_slug.find('\u{fffd}').unwrap_or(clean_slug.len()));
        let mut prefix = &clean_slug[..prefix_end];
        if let Some(last_hyphen) = prefix.rfind('-') {
            prefix = &prefix[..=last_hyphen];
        }

        let last_rep = clean_slug
            .rfind('\u{fffd}')
            .map(|i| i + '\u{fffd}'.len_utf8())
            .unwrap_or(0);
        let raw_suffix = &clean_slug[last_rep..];
        let clean_suffix = raw_suffix
            .find('-')
            .map(|idx| &raw_suffix[idx..])
            .unwrap_or(raw_suffix);

        if !prefix.is_empty() && prefix.contains('-') && fslug.starts_with(prefix) {
            if !clean_suffix.is_empty()
                && (fslug.ends_with(clean_suffix) || fslug.contains(clean_suffix))
            {
                return Some(fname.clone());
            }
            if best_match.is_none() {
                best_match = Some(fname.clone());
            }
        }
    }
...
pub async fn public_note_index_handler(
    State(state): State<ServerState>,
    SafePath(note_id): SafePath<String>,
) -> impl axum::response::IntoResponse {
...
pub async fn public_preview_handler(
    State(state): State<ServerState>,
    SafePath((note_id, file_slug)): SafePath<(String, String)>,
) -> impl axum::response::IntoResponse {
    let filename = match resolve_public_filename(&state, &note_id, &file_slug) {
        Some(f) => f,
        None => {
            return (
                StatusCode::NOT_FOUND,
...
mod isolation_tests {
    ...
    #[tokio::test]
    async fn test_public_preview_mangled_utf8_url_fuzzy_matches() {
        use axum::body::Body;
        use axum::{routing::get, Router};
        use http_body_util::BodyExt;
        use tower::ServiceExt;
        ...
        db.create_note("Hermes", "Hermes Note", None).unwrap();
        db.set_note_public("Hermes", true).unwrap();
        let md_dir = tmp.path().join(".rune/notes/Hermes/markdown");
        std::fs::create_dir_all(&md_dir).unwrap();
        std::fs::write(
            md_dir.join("Hermes-Agent-進階功能-Cron-MCP-Skin.md"),
            "# Hermes Agent Features\nSome content",
        )
        .unwrap();
        db.set_file_public("Hermes", "Hermes-Agent-進階功能-Cron-MCP-Skin.md", true)
            .unwrap();
        ...
        // Access with corrupted percent-encoding URL %9032%968E%529F%80FD
        let req = axum::http::Request::builder()
            .method("GET")
            .uri("/notes/Hermes/Hermes-Agent-%9032%968E%529F%80FD-Cron-MCP-Skin")
            .body(Body::empty())
            .unwrap();
        ...
        let body = String::from_utf8_lossy(&resp.into_body().collect().await.unwrap().to_bytes())
            .to_string();
        assert!(
            body.contains("Hermes-Agent-") || body.contains("Hermes"),
            "Should resolve fuzzy match; got: {}",
            body
        );
    }
